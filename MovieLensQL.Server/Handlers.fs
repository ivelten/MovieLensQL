...
module HttpHandlers =
    ...
    let private graphiQL (next : HttpFunc) (ctx : HttpContext) = task {
        ...
        | Some query, Some variables ->
            ...
            let result = Schema.executor.AsyncExecute(query, variables = variables, data = Schema.root requestId) |> Async.RunSynchronously
            printfn "Result metadata: %A" result.Metadata
            return! okWithStr (json result) next ctx
        | Some query, None ->
            printfn "Received query: %s" query
            let query = query |> removeSpacesAndNewLines
            let result = Schema.executor.AsyncExecute(query, data = Schema.root requestId) |> Async.RunSynchronously
            printfn "Result metadata: %A" result.Metadata
            return! okWithStr (json result) next ctx
        | None, _ ->
